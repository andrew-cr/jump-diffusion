...
class UNetVideoModel(nn.Module):
    ...
    def __init__(
        ...
        if jump_diffusion and hasattr(self, 'only_use_neighbours') and not self.only_use_neighbours:
            self.jump_diffusion_max_dim = jump_diffusion_max_dim
            self.current_dim_embedder = nn.Linear(jump_diffusion_max_dim, 512)
            self.middle_preaggregate_conv = nn.Sequential(
                Rearrange("b t c h w-> (b h w) c t"),
                nn.Conv1d(512, 512, kernel_size=3, padding=1),
                SiLU(),
                Rearrange("(b h w) c t -> b t c h w", h=1, w=1),
            )
            self.middle_postaggregate_conv = nn.Sequential(
                nn.Conv1d(512, 512, kernel_size=3, padding=1),
                SiLU(),
                nn.Conv1d(512, 512, kernel_size=3, padding=1),
                SiLU(),
            ...
            self.insertion_pred_conv = nn.Conv1d(512, 1, kernel_size=2, padding=1)
            self.x0_dim_pred = nn.Sequential(
                nn.Linear(512, 512),
                SiLU(),
                nn.Linear(512, jump_diffusion_max_dim),
            )
        self.detach_jump_grads = detach_jump_grads
    ...
    def forward(self, x, *, x0, timesteps, frame_indices=None,
        ...
        """
        B, T, C, H, W = x.shape
        # print(timesteps.view(B, -1).shape)
        timesteps = timesteps.view(B, -1)[:, :T]# view(-1, 1).expand(B, T)
        ...
        if self.return_middle:
            return rearrange(h, "(b t) c h w -> b t c h w", t=T)
        # probe middle block to do jump stuff -----------
        if self.jump_diffusion and hasattr(self, 'only_use_neighbours') and not self.only_use_neighbours:
            middle_h = h.view(B, T, *h.shape[1:])  # B x T x C x H x W
            if self.detach_jump_grads:
                middle_h = middle_h.detach()
            current_dims = (obs_mask+latent_mask).sum(dim=(1,))  # B
            current_dim_embed = th.zeros(B, self.jump_diffusion_max_dim).to(x.device)
            for i in range(B):
                current_dim_embed[i, int(current_dims[i].item())-1] = 1
            middle_h = self.current_dim_embedder(current_dim_embed).view(B, 1, 512, 1, 1)
            middle_h = middle_h * (obs_mask+latent_mask)  # set marginalised to 0
            middle_h = self.middle_preaggregate_conv(middle_h)
            middle_h = middle_h.mean(dim=(3, 4))  # B x T x C
            middle_h = self.middle_postaggregate_conv(middle_h.permute(0, 2, 1))  # B x C x T
            x0_dim_pred = self.x0_dim_pred(middle_h.logsumexp(dim=2))  # B x max_dim
            insertion_probs = self.insertion_pred_conv(middle_h).squeeze(1)  # B x (T+1)
            max_dim = x0_dim_pred.shape[1]
            padding = -float('inf') * th.ones_like(insertion_probs[:, :1], device=x.device).expand(B, max_dim+1-insertion_probs.shape[1]).to(x.device)
            insertion_probs = th.cat([insertion_probs, padding], dim=1)
        # -----------------------------------------------
        ...
        out = self.out(h)
        if self.jump_diffusion and hasattr(self, 'only_use_neighbours') and not self.only_use_neighbours:
            return out.view(B, T, self.out_channels, H, W), attns, insertion_probs, x0_dim_pred
        else:
